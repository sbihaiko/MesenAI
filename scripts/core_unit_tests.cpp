...
void TestALaterTileNeverEvictsAnEarlierOneFromItsChrPageSlot()
{
	...
	Check(MesenSheets::PlaceTileOnChrPage(bank, 1, 95, &first) && page[95] == &first, "#460: a tile takes the slot of its CHR index");
	bool placed = MesenSheets::PlaceTileOnChrPage(bank, 1, 95, &later);
	...
	Check(MesenSheets::PlaceTileOnChrPage(bank, 1, 95, &first) && page[95] == &first && page[1] == nullptr, "#460: re-filing a tile already in its slot changes nothing");
	Check(MesenSheets::PlaceTileOnChrPage(bank, 1, -1, &third) && page[1] == &third, "#460: a loaded CHR RAM tile (index -1) takes the first free slot");
}
...
void TestADisplacedTileGoesToTheLeastFilledColumnOfItsBank()
{
	...
	MesenSheets::PlaceTileOnChrPage(bank, 2, 0, &b0);
	MesenSheets::PlaceTileOnChrPage(bank, 2, 1, &b1);
	MesenSheets::PlaceTileOnChrPage(bank, 1, 95, &first);
	bool placed = MesenSheets::PlaceTileOnChrPage(bank, 1, 95, &later);
...
void TestAFullChrPageRefusesATileInsteadOfEvictingOne()
{
	...
	bank[1] = std::vector<Tile*>(256, nullptr);
	for(int i = 0; i < 256; i++) {
		MesenSheets::PlaceTileOnChrPage(bank, 1, i, &tiles[i]);
	}
	bool placed = MesenSheets::PlaceTileOnChrPage(bank, 1, 7, &tiles[256]);
