...
Implemented 2026-09-17 (P.1-local, ADR-0206): the Decision's "MepPackManager for local containers" — a stamp-less local drop takes its tree `content_id` from the cache at `EnhancementPacks/.cache/content-ids.json`, computed off the ROM-load path by a fingerprint-validated background refresh, and a container whose `content_id` matches a stamped sibling's adopts that `pack_id`. `ComputeFolder` gained the `.mep-source` control-file skip (ADR-0206 §2) and now drops the ADR-0139-excluded entries before reading them (same result: `ComputeTree` already excluded them). Still not implemented: the CI-side `content_id` is the tree form only (the recipe composite is computed only by the Core installer).
