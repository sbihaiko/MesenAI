...
- **The library job does not mint at frame 361.** `scripts/record_library.sh`
  ...
  Ending a mint where its script ends was a tooling fix (#465), not a change to
  this set. **Fixed 2026-09-25:** the job now runs a mint for the smallest
  whole number of seconds that covers its script (6 s here), so its
  `stage1-run.mss` stops at frame 362 (target 361) with the same RAM as the
  hand mint above, and a 60 s job run keeps 3 555 frames
