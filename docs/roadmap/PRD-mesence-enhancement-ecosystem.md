...
- **F9.21 withdrawn** (2026-09-12, ADR-0180 superseded by ADR-0179 §4):
  rigid `parts[]` recurring across poses were measured on the kit
  (`runs/golden-20260912/spike-pose-parts.md`) and the best cover tiles
  21–43 % of the kept poses with 1.0–1.4 parts each — the "part" is the
  whole figure inside its projectile variant; genuine limb parts tile 10 of
  37 Contra stage-1 poses and none on the three other golden games. No
  `parts[]` block ships; `variantOf` carries what the data supports. The
  door reopens as a measurement (a game whose cover tiles a majority of
  its poses with >= 2 parts), not as a slice.
...
(this section is the review's recommendations, written as slices so each
can be run and closed on its own). **Nothing here is accepted yet**; the
user picks the order and the slices to run. No ADR is needed for C.2, C.3
...
- **Rule drift.** ADR-0189 and ADR-0190 were accepted and implemented in
  the same turn, against `CLAUDE.md` ("accepting one is a request for work
  ... get a go-ahead before implementing it in the same turn"). The
  precedent is now two deep.
...
| An ADR is accepted and implemented in the same turn (ADR-0189, ADR-0190) | `CLAUDE.md` rule stands: accepting is a request for work, and the go-ahead is quoted in the ADR Status line **and** the PR body. Two precedents exist; a third is a rule change, and rule changes are made in `CLAUDE.md`, not by repetition |
