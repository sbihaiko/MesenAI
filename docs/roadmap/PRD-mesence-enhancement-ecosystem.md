...
| F12.13 | **A variant may not add content the capture lacks (ADR-0221, option B).** In `MesenSheets::SelectScreenAnchors`, after the `kAnchorVariantAgree` test, a frame stays a *variant* of the captured screen only when every cell it changes is non-empty in the captured frame too; a frame that draws content into a cell the capture holds empty (a flat tile) is a *rival*, whatever its agreement ratio, and the discrimination pass must separate it. Recorder-only: `CaptureScreen`, numbering, ADR-0156 residency and the render path are untouched; option D was not picked. | **Accepted 2026-09-22, not implemented** — user's picks, verbatim: *"B sozinha (Recommended)"*, *"Não, só a ADR"*. Prerequisite: none (ADR-0217/0218 shipped). Bounded input: Punch-Out!! re-recorded from the same 60 s route as `docs/validation/adr0221-capture-overdraw-harness-2026-09-20.md`. Stop when (1) `scripts/measure_capture_overdraw.py --sweep` reports **0 erased cells** on all 36 frames, including the 41–43 s occurrence; (2) unit tests cover the kind test on a synthetic `GridFrame` pair (addition → rival, content-for-content → variant, below-threshold → rival); (3) the 30-ROM bounded library is re-recorded (~15 min) and the log publishes capture count **and** per-capture draw rate before/after — the cost option C would have hidden is made visible here, and a drop in draw rate is reported, not a stop. Re-measures issue #339 (closed as tracked here). |
| F12.14 | **The OAM dump is self-describing (ADR-0222, option A).** `MESEN_OAM_STREAM_DUMP` interns shapes and palettes with `K`/`P` lines like the grid dump and writes each sprite as `<shape>,<x>,<y>,<pal>` — the `ShapeId` `RecordOamEntry` already holds plus a new palette id on `OamEntry`, compared by `SameEntries`. `mep_conditions.py` gains `parse_oam_dump`; `spriteNearby`, `spriteAtPosition`, `positionCheckX/Y` and `originPositionCheckX/Y` get verdicts, and `memoryCheck` reads its second operand from the `M` line. ADR-0169's viewer wire format gains the same byte (`make capture-tool`); `scripts/stages/README.md` and ADR-0197 §3's sprite bullets are amended in the same PR. | **Accepted 2026-09-22, not implemented** — user's picks, verbatim: *"A: dump autodescritivo (Recommended)"*, *"So registrar (Recommended)"*. Prerequisite: none. Bounded input: the F12.6a Contra pack and its 60 s route, re-recorded. Stop when (1) the authored `nearThePlayer,spriteNearby,…` line reports a verdict (`always held` / `mixed` / `never held`), never `not evaluable`; (2) a two-address `memoryCheck` on the same route reports a verdict from the `M` plane; (3) unit tests cover the writer (a synthetic `OamFrame` pair differing only in palette does not collapse) and the parser (a `K`-less dump reports `not evaluable: OAM stream carries no tile data`); (4) a pre-F12.14 recording still parses and reports that reason. Re-measures ADR-0197 §2's promise on the sprite condition the recorder itself emits (ADR-0189). |
