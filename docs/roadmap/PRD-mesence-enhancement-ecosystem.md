...
| Slice | Deliverable | Decision |
|---|---|---|
| F12.11 | **Layered surface for the paint program (OpenRaster).** Beside every surface PNG the kit writes `<name>.ora` — a zip with `stack.xml`, `mergedimage.png`, `Thumbnails/thumbnail.png` and one PNG per layer, written with `zipfile` + `xml.etree` and the PNG writer the generators already have. Layers, bottom to top — **five on a recorded surface, four on an F12.9 static page**: `orig` (the `*.orig.png` twin, `edit-locked`), `context` (the 1x stitched-map crop around a figure at 50 % opacity — only when a recording exists, absent on F12.9 pages), `paint` (fully transparent, the **selected** layer, the only one the artist touches), `guides` (cell grid, pose / cycle captions from `names.json` or the sidecar ids, hatch over `seen: false` cells — drawn in one sentinel colour outside every NES palette, `visibility="hidden"` for export), `palettes` (a swatch strip of the palettes recorded for that sheet, hidden). GIMP, Krita and MyPaint open `.ora` natively; Photoshop and Aseprite do not and stay on F12.4's per-layer asset names — **no `.psd` or `.aseprite` writer**, stated in `docs/remastering-a-game.md`. F12.11 is a second path beside F12.4, not its replacement: the artist evidence measured so far (Metroid, a spreadsheet user) does not show a GIMP/Krita population, so F12.4 stays the default path and this one is measured against it. **The return path does not change:** the artist exports a flat PNG over the F12.4 name; `sheet_repaint` keeps only cells that differ from `orig`, and `mep_lint.py` fails a cell that contains the sentinel colour (the guides layer was left visible) naming the cell. | **ADR-0220 accepted 2026-09-22 (*"aceito o F12.11. nao implemente ainda."*) — slice open, not started, no go-ahead to build.** The ADR was needed before start because it adds a fifth file kind to ADR-0183 §2's surfaces and fixes the layer contract; it must also state that `.ora` is **write-only** for the toolchain (reading `paint` out of it is stdlib-trivial and is refused on purpose, or the sheet stops being the source of truth). Prerequisite chain, in full: F12.3 (the reload that shows it) → F12.4 (the name the flat export lands on) → F12.11; the SMB bounded input additionally needs F12.9. Bounded input: one Contra figure sheet (recorded, five layers) and one SMB static page from F12.9 (four layers). Stop when (1) `stack.xml` validates against the OpenRaster 0.0.5 schema shape the three programs read and each `.ora` round-trips through `zipfile` unchanged; (2) GIMP and Krita open both files with every layer named (five and four respectively) and `paint` selected — this row is logged by a person, per this phase's cold-read rule; (3) a stroke on `paint`, exported flat, reaches the game pixel-exact via F12.3 with the unchanged cells dropped; (4) the same export with `guides` left visible is refused by lint with the offending cell named. What we measure is ours: file validity, layer order, refusal, pixel-exact result. Re-measures "Painting, end to end" and the **"simple"** constraint: open one file, paint, export, look at the game. |
| F12.15 | **A recorded screen does not hide a behind-background sprite over empty canvas (ADR-0224).** New hires.txt tag `<bgPreservesBehindBgSprites>` (MesenAI extension; unknown tags are skipped in silence by every Mesen loader, unlike `<options>` tokens, which fail the load). With the tag, `HdNesPack::GetPixels` leaves a pixel to the behind-background sprite pass when the sprite pixel is opaque, the ROM's background pixel is colour 0, and a priority-20–29 background would otherwise paint it; without the tag the output is byte-identical to today. `HdPackBuilder` writes the tag on every recorded pack, bootstrap included; hand-written and community packs are untouched. `scripts/measure_capture_overdraw.py` splits its count into `erased background` / `erased sprite` from the OAM dump (ADR-0222) or the `M` RAM line. Spec entry in the hires.txt extension draft (ADR-0004), one passage in `docs/remastering-a-game.md` and the kit's `ARTIST.md`; `mep_lint.py` accepts the tag. | **Accepted 2026-09-22, in progress the same day** — user's picks, verbatim: *"Opt-in por pack (Recommended)"*, *"Sim, na mesma fatia (Recommended)"*, *"So a ADR agora (Recommended)"*, *"Tag nova em hires.txt (Recommended)"*; build go-ahead verbatim *"libera a F12.15, dispara as três partes em paralelo. mergea o PR assim que puder e garante que t  tudo na main."* Prerequisite: none (F12.14's OAM dump is on `main`). Bounded input: Punch-Out!! re-recorded from ADR-0221's 60 s route, tag written by the recorder. Stop when (1) the split tool reports **0 `erased sprite`** on the 41–45 s frames with the tag and the same non-zero figures as F12.13's log without it; (2) unit tests cover the tag's parse, the writer, and both renderer paths (tag off → byte-identical pixel; tag on → sprite kept over colour 0, hidden under an opaque background pixel, front sprites unaffected); (3) library packs render byte-identically **without** the tag (no regression for community packs) and the per-frame render cost with the tag is published, not assumed; (4) the card's `erased background` figure is unchanged (that is F12.16's job). Re-measures issue #339's second cause. |
| F12.16 | **Emptiness probes as a last anchor pass (ADR-0223, option A).** In `HdPackBuilder::CaptureScreen` flat runs stay in the candidate list, expanded to every cell they cover and marked `Usage = UINT32_MAX` so ADR-0050's rarity pools ignore them; in `SelectScreenAnchors`, only when rivals survive the stable and wide passes, one more greedy pass runs over the stable pool plus the flat cells no variant changes, kept only if it separates more rivals. Recorder-only; ADR-0221's kind test is the prerequisite that makes the addition frames rivals. | **Accepted 2026-09-22, not implemented, sequenced after F12.15** — user's picks, verbatim: *"A: probes como ultimo passo (Recommended)"*, *"Depois da F12.15 (Recommended)"*. Prerequisite: F12.15's split overdraw tool, so the measure is `erased background` alone. Bounded input: Punch-Out!! on ADR-0221's 60 s route. Stop when (1) `erased background` = **0** on the card (19–27 s), the fight window's `erased sprite` column not counted against this slice; (2) unit tests on a synthetic `GridFrame` pair where only a flat cell separates capture from rival; (3) the 30-ROM library re-recorded with capture count, draw rate (`scripts/measure_capture_draw_rate.py`) and never-firing count published before/after — the prototype's 10 → 9 captures and 0.1200 → 0.0865 on Punch-Out!! is the expected shape of the cost, reported, not a stop. Re-measures issue #339's first cause. |
...
**accepted on 2026-09-22** without a go-ahead to build, so F12.11 is open and
not started. Each slice is one task, and a
