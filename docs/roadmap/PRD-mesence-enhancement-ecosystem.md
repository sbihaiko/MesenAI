...
- **`poses.json` says what follows what** (2026-09-12, F9.20, ADR-0179):
  the recorder links kept clusters frame to frame into tracks (nearest
  top-left within 16 px), and each entry gains `hold` and `next[]`; the
  tracks are scanned for `cycles[]` (a period repeated twice in a row, found
  on the track *sequence* so one silhouette can be two phases) and for
  `sequences[]` (a non-looping run seen twice); a kept pose that is another
  kept pose plus fewer than 4 tiles is labelled `variantOf` it. The editor
  ranks the base above its variants and lays the seed list out one cycle at
  a time in phase order, captioned `cycle000 3/6`. Measured on the Contra
  golden re-record: the soldier's run is one period-6 cycle (two 3-cycles
  the soldier alternates between), the somersault a 4-cycle, the death an
  8-pose sequence, and exactly the 9 variants the spike predicted. The
  player's run is *not* a cycle here — no track holds it for two full
  turns, because the entry script taps R in short bursts — which is the
  measurement F9.22 exists to fix, not a defect of the rule. Five new
  `core_unit_tests` cases from ADR-0179 §6, three Python tests; the pack
  format is unchanged for a reader that ignores the new optional fields.

- **Per-stage recording, the tooling half** (2026-09-12, F9.22): the
  headless recorder writes a save state at the end of a complete run
  (`save-state=`) and starts from one (`state=`, already there), and
  `scripts/record_stages.sh` runs one short bootstrap record per state into
  its own pack folder. On Contra the stage-1 state plus a 60 s long-run
  script finds the player's run as a period-6 cycle at once — the exact
  loop the 120 s golden run only ever showed as 3–4-pose fragments — which
  is the measurement the slice was opened for. Run on the kit the same
  day: Mega Man 3's run, Link's walk in four directions and Excitebike's
  wheels all come out as cycles from a 60 s stage-1 run. Contra's later
  stages turned out not to need a human: input scripts chained from state
  to state, steered by RAM read off the `.mss` (`scripts/mss_ram.py`),
  crossed stage 1, beat the wall and minted `stage1-boss` and `stage2-base`
...
- **Water and the second player** (2026-09-13, F9.22): the input script
  grammar gained a port-2 token — `<count>f <port1>|<port2>`, ADR-0157
  amended — and `headless_record` plugs a pad into port 2 only when a
  line names one, so every one-player script records as before. Two
  stage-1 states minted headlessly: `stage1-water` (Bill in the water at
  the start: 13 poses, no cycle — he swims and shoots without animating)
  and `stage1-2p` (the 30-lives code, Select, Start: 311 poses, 11 cycles,
  two of them the pair running side by side as one 20-tile fused pose).
  `stage1-2p-probe` moves only Lance and reads his run as `port2` (13/14
  and 12/12 windows) — the first measurement of the port-2 path. Bill and
  Lance share every pose (shape-keyed vocabulary, palette-only
  difference), so a probe that moves both leaves the run unclassified
  (7/13 and 6/13), as the rule intends. Two `core_unit_tests` cases.

- **The base, the waterfall and its boss** (2026-09-13, F9.22): Contra
  stages 2 and 3 beaten headlessly, states `stage3-waterfall`,
  `stage3-boss` and `stage4-base` minted and recorded with a probe each
  (`scripts/stages/README.md` has the RAM map and the search shapes).
  What the RAM taught: an enemy slot is reused, so a target is (slot,
  type); the base head's HP nibble is not monotone, so the win is read off
  `$003b`; the vertical stage's camera catches up only after a landing, so
  every window ends idle; the stage-3 core's visible HP flips to 0xF1 while
  its mouth is shut and the lasting value is in a second table (`$05c5`).
  One search failure worth keeping: the greedy climb rule (highest camera,
  then highest Bill) cannot take an exit that is below-left of the figure;
...
- **The second base, to its boss room** (2026-09-13, F9.22 closed):
  stage 4 played through eight screens by the same search and `stage4-boss`
  recorded from the boss room with a probe. What the second base taught:
  bullets converge toward the vanishing point, so off-centre cores are hit
  from lanes 24–48 px farther out; core HP nibbles regenerate, so only the
  cores-left counter is monotone; a wall's core may appear only after its
  cannons die, so targets are re-read per state. The chain from
  `stage4-base` is versioned; the boss (pods and eye down, head not found
  within the depth budget) and stages 5–8 are left unplayed on purpose —
  the states exist to measure the format, and what is recorded already
  shows every mechanism it has to carry.

- **`driver` by interruption** (2026-09-13, ADR-0181 §3, F9.23):
  `FindPoseRuns` reads each cycle occurrence as a window that stops at its
  last phase advance plus that phase's median hold, and a port drives the
  cycle when >= `kDriverMinWindows` (4) windows exist, >= 2/3 of them stop
  within `kDriverStopLag` (12) frames of a release on that port and the
  other port fails the same test; `cycles[]` entries gain `"driver":
  "port1"|"port2"` only then, the save line says how many. One probe
  script per golden game (`stage1-probe.txt`: hold a direction >= 2 turns,
  release, idle, no scroll) is the measurement's input; on the entry and
  stage scripts nothing is attributed, by design. Measured: Contra's run
  right/left `port1` (12/12 each), soldier none; Zelda's four walks
  `port1` (>= 9/10); Mega Man 3's run `port1` (12/13), enemies none;
...
| F9.20 | **Pose succession in the sidecar** — at save time the recorder links each kept cluster to the nearest kept cluster of the next retained frame (<= 16 px), and `poses.json` gains per-entry `next[]`/`hold`, top-level `cycles[]` (a period that repeats >= 2 times on one track; found on the track *sequence*, because one silhouette can serve two phases — Contra's player run is `020 003 022 023 003 024`, period 6, and reads as a 3-cycle on a first-order graph) and `sequences[]` (a non-looping run seen >= 2 times: the death animation), plus `variantOf` for a kept pose that is another kept pose + < 4 tiles (figure + projectile, 9 of 64 Contra entries). The composition editor lays the sprite side out by cycle: one row per loop, columns in phase order — the artist's grid. Host-free in `SpriteGrouping`, `core_unit_tests`; no change to capture, identity, thresholds or `hires.txt` | **shipped 2026-09-12** — ADR-0179 accepted by the user and implemented the same day (§3). On the Contra golden re-record: 690 tracks, 5 cycles, 8 sequences, the 9 variants the spike predicted; the soldier's run comes out as *one* period-6 cycle (`000 011 010 000 016 008`, 5 repeats, hold 8), the somersault as the 4-cycle (4 repeats), the death as an 8-pose sequence seen twice; the player's period-6 run is **not** a cycle in this recording — the entry script never holds R for two full turns on one track (longest: 8 phases), so it appears as four 3–4-pose sequences. That is the ADR's own consequence ("a cycle is what the recording showed") and F9.22's job. Spike `runs/golden-20260912/spike-pose-succession.md` (not versioned); evidence path: env-gated `MESEN_OAM_STREAM_DUMP` in `HdPackBuilder::WriteSpriteSheets`, sibling of `MESEN_SHEET_GRID_DUMP` |
| F9.21 | **Parts** — rigid tile subsets that recur across >= 2 kept poses become `parts[]`, each pose cites its `composition[]` (part + translation) and a `rest` count; `tiles[]` stays authoritative. The editor shows a pose whole and its parts as labelled regions, joined to the `sprNNN` sheet that stores them. Makes ADR-0171's storage-once rule visible: the artist paints the legs once and sees every figure change, as `BillRizer.png` does with its 3 x 6 grid | **withdrawn 2026-09-12** — ADR-0180 superseded by ADR-0179 §4 on the kit measurement (`runs/golden-20260912/spike-pose-parts.md`): the best cover tiles 21–43 % of the poses, almost all with the whole figure as the part (`variantOf` under another name); genuine limb parts cover 10 of 37 Contra stage-1 poses and none on the other three games. Reopens only as a measurement, if a game's `nofig`+`conn` cover tiles a majority of its poses with >= 2 parts |
| F9.22 | **Per-stage recording roadmap** — the retained OAM stream is capped at 4096 frames (`kMaxSheetFrames`, ~11.5 MB), and gameplay almost never repeats a frame: the 120 s Contra golden run kept 4151 of 7213 emulated frames, so the last 51 s produced no poses and no adjacency. One long run is the wrong shape. Per golden game: a set of `headless_record` save states (`state=<file.mss>`, one per stage / distinct area) plus a <= 60 s entry script each — one run per state into its own pack folder, judged as a union (merging retained streams across runs into one pack is a decision not yet taken and not assumed here), so every subject the artist pack covers (Contra: 8 stages, water, base interior, bosses, player 2) is on screen at least once. Deliverable is the state files, the scripts and a `make`-able batch, not code in the emulator | **tooling shipped 2026-09-12, coverage closed 2026-09-13** — `headless_record` gained `save-state=<file.mss>` (written only when the run reaches its frame target) to pair with `state=`, and `scripts/record_stages.sh <rom> <stages-dir> <out-dir> [seconds]` runs one bootstrap record per `<stage>.mss` + `<stage>.txt` into its own pack folder. Proven on Contra stage 1: a state minted from the entry script and a 60 s script that holds R for 240 f at a time yields the player's period-6 run cycle (`004 015 012 004 016 014`, 10-tile poses, hold 8, 2 repeats — soldiers running into the player turn his cluster into a fusion and end the track, which is why the count stays low) next to the soldier's own period-6 cycle (8-tile poses); the 120 s golden run never showed the player's run as a cycle at all. Two `headless_record` defects surfaced on the way and were fixed the same day: `state=` restored the emulator's frame counter while both the run target and the input script were indexed by absolute frame, so a run from a state ended early and its script was skipped by the state's age. Run on the whole kit the same day from stage-1 states minted by the entry scripts (`scripts/stages/<game>/`, versioned; states under `runs/`, unversioned — a CHR-RAM state carries ROM-derived graphics): Mega Man 3 7 cycles (the run as `001 002 001 003`, period 4 with the middle frame twice, 34 repeats), Zelda 1 15 (Link's two-frame walk in every direction, hold 6, 51–80 repeats), Excitebike 8 (the wheels, 398 repeats), Contra 2 (the player's run and the soldier's, both period 6). Cycles with period > 6 and a hold near 100 are echoes of the script's own loop, not animations — a reader should weigh `repeats` and `hold` together. Contra's later stages are reached **headlessly, not in the GUI**: the game is played through `headless_record` input scripts chained from state to state, with progress and survival read off the save state's RAM (`scripts/mss_ram.py`: lives `$0032`, screen `$0064`, stage `$0030`, player X/Y `$0334`/`$031A`, player state `$0090`, the stage-1 core's HP `$0585`). A greedy explorer crossed stage 1 on the scroll position; a depth-first search over 60 f input windows beat the wall (prone bursts from the small platform are the only shots at the core's height — standing shots pass above it from the platform and below it from the ground). States minted 2026-09-12: `stage1-boss` (the wall, 69 poses, 3 cycles) and `stage2-base` (the corridor, 163 poses, 10 cycles — the base soldier's run as three period-3 cycles of 8-tile poses, hold 4). Water and player 2 recorded 2026-09-13 (`stage1-water` 13 poses; `stage1-2p` 311 poses, 11 cycles; the input script grammar gained `<port1>|<port2>` and `stage1-2p-probe` reads Lance's run as `port2`). Stages 2 and 3 beaten headlessly 2026-09-13 (base: targets are the static slots — cannons type 19, core type 20 — read off `$0528+i`/`$0578+i`, survival-constrained DFS with a death budget; waterfall: progress `$0064*256+$0065`, greedy jump explorer plus four hand-chained jumps left where the exit is below-left; its boss: arms slots 5–6 HP 16, core's lasting HP `$05c5`, Up+B from x = 100–128 while the mouth is open, 105 windows, no death). Three more states and packs: `stage3-waterfall` 252 poses / 12 cycles, probe `port1` both ways; `stage3-boss` 517 poses / 12 cycles, probe reads Bill's run `port1`; `stage4-base` 291 poses / 17 cycles (the base soldiers' forward run, 37 repeats; its probe attributes nothing — Bill's sideways walk in the base shows up as period-5 cycles of 4 repeats, under the 4-window floor). Stage 4 (the second base) played through to its boss room the same day and `stage4-boss` recorded from there (181 poses / 12 cycles, probe 189 / 7, no driver — the room never leaves Bill a clean track); the bullets' convergence, regenerating core HP and cores that appear only after their cannons die are in the README. The stage-4 boss itself is not beaten (pods and eye down, one core left, the head not located within the search's depth budget) and stages 5–8 are **not planned**: two bases, the waterfall and its boss already exercise every mechanism the format has to carry, and a further stage costs hours of search for no format decision. Coverage closes here |
| F9.23 | **`driver` by interruption** (ADR-0181 §3, accepted 2026-09-12) — a cycle is attributed to the port whose button release stops it: >= `kDriverMinWindows` windows, >= `kDriverStopShare` of their ends within `kDriverStopLag` frames of a release on that port, and the other port failing the same test; no `driver` otherwise, nothing deleted (§5). Ships with one probe script per golden game (hold / release / idle) because the entry scripts hold one button most of the run and yield nothing to measure. Acceptance: Contra's player run gets `port1` and the soldier's does not; Excitebike's wheels get no `driver` | **shipped 2026-09-13** — probe scripts `scripts/stages/<game>/stage1-probe.txt` for the four golden games; the rule in `SpriteGrouping::FindPoseRuns` (a window stops at its last advance + that phase's median hold), `driver` on `cycles[]`, constants 4 / 12 f / 2/3 confirmed by the measurement (ADR-0181 §4): Contra's run right and left `port1` 12/12 each, soldier none; Zelda's four walks `port1` 10/10, 10/10, 9/10, 10/10; Mega Man 3's run `port1` 12/13, its two enemy cycles 0/13 and 2/13 none; Excitebike's wheels 3/18 none |
...
3b. **Phase 9 F9.22 (closed 2026-09-13) after F9.23**, F9.20 shipped
   ...
   are not planned (§3). F9.22 is closed as coverage: the per-stage
   recording tooling, six Contra states with a chain file each where the
