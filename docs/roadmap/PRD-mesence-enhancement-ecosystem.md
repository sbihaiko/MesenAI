...
3b. **Phase 9 F9.22 (states pending) after F9.23**, F9.20 shipped
   ...
   `stage4-base` recorded the same day (§3). What stays open in F9.22 is
   coverage: Contra stages 4–8 with the headless chain tooling (the base
   solver is running on stage 4), and probe scripts for any later state
   whose cycles should carry a `driver`.
