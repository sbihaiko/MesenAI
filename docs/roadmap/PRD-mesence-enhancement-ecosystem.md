...
| F14.10 | **A probe's evidence is the tile the run time reads (ADR-0235 option 2, #499).** At capture time the recorder records, for each anchor probe, the tile its absolute pixel reads the way `HdPackTileAtPositionCondition` does — so a row fetched at another fine scroll than the frame's dominant `FineX` (a fixed status bar) is no longer read one tile off; a gate that then keeps a rival and has no separating stable probe is refused. Packs already written are not re-recorded. | Accepted 2026-09-25; go-ahead verbatim *"pode implementar a F14.10 com o DeepSeek"* (2026-09-25), **in progress**. Stop when (1) Ninja Gaiden `stage1-run`'s 31 s frame no longer draws the stale `screen001` (live HUD: TIMER/SCORE move); (2) unit tests pin a mixed-fine-scroll frame whose HUD-row probe is read at its own fetch phase; (3) the 30-ROM library sweep publishes captures, draw rate and stale frames before/after, and every game whose numbers move is explained. |
...
| 0233/0235 | **0235 accepted 2026-09-25, option 2 — pending slice F14.10**; supersedes 0233 (option A measured, not shipped; its premise that the wrongly-gated frames were never retained was falsified). Picks verbatim *"Não mergear A; medir retenção (Recommended)"*, then *"Opção 2: corrigir o gravador (Recommended)"* | a capture's anchor probe is evidenced by the tile the run time reads at its absolute pixel, not by the frame's single dominant `FineX`; a gate with a surviving rival and no separating stable probe is refused |
